...
typedef enum _REGULATORY_DOMAINS
{
	REGULATORY_FCC = 0,
	REGULATORY_EU,
	REGULATORY_MKK,
	REGULATORY_COMMON,
}E_REGULATORY_DOMAINS;
...
typedef struct WifilinkCfg
{
    ...
    */
	...
	bool Detection_network = true;//检测有线网络开关，没有有线设备设置成false

	bool isSpecialCh = true;  //base是否用特殊信道，如果不用就改成false，这时会跟着AP的信道走 ApCfg apcfg;  // hostapd.conf 初始配置信息

	bool isEthNotstartAp = false; //如果接了网线后不需要设备启动热点，改成true,默认是会发起热点
...
int WifiDelMac2MacArray(uint8_t *mac); //删除指定MAC地址的MAC
...
int GetWifiLevel(); //获取WiFi当前在第几等级

int WifiCheckBridgeInterface(char *interface);//判断接口是否有桥接上 eth0 wlan0 p2p0 成功还回0

int WifiGetBestChannel(E_REGULATORY_DOMAINS eVal);//获取最好的信道
